...
st.set_page_config(page_title="E-commerce Churn - Equipo 70", layout="wide", page_icon=logo_path)
...
def load_models():
    try:
        ...
        # KNN
        knn_model = joblib.load(get_path("modelo_knn_churn_final.pkl"))
        knn_scaler = joblib.load(get_path("scaler_knn_churn.pkl"))
        knn_umbral = joblib.load(get_path("umbral_optimo_knn.pkl"))
        knn_features = joblib.load(get_path("features_knn_churn.pkl"))
        X_test_knn_scaled, y_test_knn = joblib.load(get_path("datos_test_knn.pkl"))
        
        # XGBoost
        xgb_path = get_path("modelo_xgboost_churn.pkl")
        if os.path.exists(xgb_path):
            xgb_model = joblib.load(xgb_path)
            X_test_xgb, y_test_xgb = joblib.load(get_path("datos_test_xgboost.pkl"))
        else:
...
def load_data():
    try:
        # Intentar cargar desde la ruta relativa al script dashboard/app.py
        base_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Opciones de rutas posibles
        paths = [
            os.path.join(base_dir, "../datos/dataset_ecommerce_limpio.csv"), # Si corre desde dashboard/
            os.path.join(base_dir, "datos/dataset_ecommerce_limpio.csv"),    # Alternativa
            "datos/dataset_ecommerce_limpio.csv",                            # Si corre desde root
            "dataset_ecommerce_limpio.csv"                                   # Si está en la misma carpeta
        ]
        ...
        st.error("No se encontró el archivo de datos 'dataset_ecommerce_limpio.csv'")
        return pd.DataFrame()
